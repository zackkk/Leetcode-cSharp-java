...
public class Solution {
    // idea comes from: https://leetcode.com/problems/ugly-number-ii/
    public int NthSuperUglyNumber(int n, int[] primes) {
        ...
        for(int i = 1; i < n; i++){
            ...
            for(int j = 0; j < primes.Length; j++){
                ugly[i] = Math.Min(ugly[i], ugly[ptrs[j]] * primes[j]);        // choose min from multiple lists
            }
            for(int j = 0; j < primes.Length; j++){
                if(ugly[i] == ugly[ptrs[j]] * primes[j]){
                    ptrs[j]++;
                }
            }
        }
